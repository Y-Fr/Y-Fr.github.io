...
var gallery_main = (function($) { var _ = {
	...
	keys: {
		...
		// Escape: Toggle gallery_main wrapper.
			27: function() {
				_.toggle();
			},
	...
	initProperties: function() {
		...
		// gallery_main wrapper.
			_.$gallery_main = $('#gallery_main');

		// Toggle.
			$('<div class="toggle"></div>')
				.appendTo(_.$gallery_main);
				...
				_.$window
					.on('resize', function() {
						window.setTimeout(function() {
							_.$viewer.css('width', _.$window.width() - _.$gallery_main.width());
						}, 100);
					})
					.trigger('resize');
	...
	initEvents: function() {
		...
		// Viewer.
			...
			// Hide gallery_main wrapper on tap (<= medium only).
			...
			// Touch gestures.
				_.$gallery_main
					.on('touchstart', function(event) {
						...
						// Record start position.
							_.$gallery_main.touchPosX = event.originalEvent.touches[0].pageX;
							_.$gallery_main.touchPosY = event.originalEvent.touches[0].pageY;
					...
					.on('touchmove', function(event) {
						...
						// No start position recorded? Bail.
							if (_.$gallery_main.touchPosX === null
							||	_.$gallery_main.touchPosY === null)
								return;

						// Calculate stuff.
							var	diffX = _.$gallery_main.touchPosX - event.originalEvent.touches[0].pageX,
								diffY = _.$gallery_main.touchPosY - event.originalEvent.touches[0].pageY;
								boundary = 20,
								delta = 50,
								result = false;

						// Swipe to close.
							switch (_.settings.gallery_mainSide) {
						...
						// Overscroll fix.
							var	th = _.$gallery_main.outerHeight(),
								ts = (_.$gallery_main.get(0).scrollHeight - _.$gallery_main.scrollTop());

							if ((_.$gallery_main.scrollTop() <= 0 && diffY < 0)
							|| (ts > (th - 2) && ts < (th + 2) && diffY > 0)) {
	...
	initViewer: function() {
		...
		// Create slides from gallery_thumbnails.
			_.$gallery_thumbnails.children()
				.each(function() {
	...
	switchTo: function(index, noHide) {
		...
		// Hide gallery_main wrapper if medium is active.
			if (!noHide
	...
	up: function() {
		...
		// Calculate new index.
			var i, c = _.current, l = _.slides.length, tpr = _.settings.gallery_thumbnailsPerRow;
	...
	/**
	 * Shows the gallery_main wrapper.
	 */
	show: function() {
		...
		// Show gallery_main wrapper.
			_.$body.removeClass('fullscreen');

		// Focus.
			_.$gallery_main.focus();

	},

	/**
	 * Hides the gallery_main wrapper.
	 */
	hide: function() {
...
}; return _; })(jQuery); gallery_main.init();
